...
class iLQR():
    ...
    def compute_nom_npc_traj_distribution(self, l_x, l_xx, l_u, l_uu, l_ux, c_x, c_xx, reg=1e-2):
        ...
        Sigma_u_inv = (self.Su.T @ (self.Qs+self.Cxx) @ self.Su) + self.Rs
        delta_us = -np.linalg.solve(Sigma_u_inv, self.Su.T @ (self.Qs+self.Cxx) @ self.Sx @ -np.zeros(self.Dx) 
                                    + 0.5 * (self.Su.T @ self.Cx).flatten())+ lx_flat @ self.Su + lu_flat
        delta_xs = self.Sx @ np.zeros(self.Dx) + self.Su @ delta_us
        Sigma_u = np.linalg.inv(Sigma_u_inv)
        Sigma_x = self.Su @ Sigma_u @ self.Su.T 
    ...
    def get_state_cost_single_conditional(self, Sigma_x, nom_traj):
