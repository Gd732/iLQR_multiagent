...
class CAR: 
    ...
    RF = 3.6  # distance from rear to vehicle front end
    RB = 1.2  # distance from rear to vehicle back end
    W = 2.5  # width of car
...
def draw_arrow(x, y, theta, L, c):    
    ...
    x_start = x+CAR.RF*np.cos(theta)
    y_start = y+CAR.RF*np.sin(theta)
    x_end = x + L * np.cos(theta)+CAR.RF*np.cos(theta)
    y_end = y + L * np.sin(theta)+CAR.RF*np.sin(theta)
